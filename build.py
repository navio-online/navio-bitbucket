...
def update_version(ver=None):
    ...
    with open('navio/meta_bitbucket.py', 'w') as f:
        f.write(file_str)

    nsh.git('commit', 'navio/meta_bitbucket.py', '-m', 'Version updated to {}'.format(ver))
...
def create_tag():
    ...
    if m:
        ...
        raise "Can't find/parse current version in './navio/meta_bitbucket.py'"
...
def pypi():
    ...
    args.append('dist/navio-bitbucket-*')
    nsh.twine(args)
